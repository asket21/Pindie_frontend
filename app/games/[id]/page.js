...
export default function GamePage(props) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      } catch (error) {///Обходим вечный прелоадер, при неверном адресе спросить как сделать иначе
        setPreloaderVisible(false)
      }
  ...
  const handleVote = async () => {
    ...
    if (isResponseOk(response)) {
      ...
      setGame(() => {
        return {
          ...game,
          users: isVoted ? game.users.filter((user) => user.id !== authContext.user.id) : [...game.users, authContext.user],
        };
      });
      setIsVoted(!isVoted);
    }
      ...
      <>
        ...
        <section className={Styles['about']}>
          ...
          <div className={Styles["about__vote"]}>
            ...
            <button disabled={!authContext.isAuth}
              className={`button ${Styles["about__vote-button"]}`}
              onClick={handleVote}
            >{!isVoted ? "Проголосовать" : "Убрать голос" }</button>
          </div>
        </section>
      </>
    ) : preloaderVisible ? (
